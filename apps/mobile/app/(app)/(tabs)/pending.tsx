  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      <FlatList
        data={messages}
        keyExtractor={i => i.id}
        refreshControl={<RefreshControl refreshing={loading} onRefresh={load} tintColor="#0FA79A" />}
        contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 24 }}
        ItemSeparatorComponent={() => <View style={{ height: 10 }} />}
        ListEmptyComponent={
          !loading ? (
            <View style={s.empty}>
              <TrayIcon size={52} color="#4E677F" weight="duotone" />
        ...
        renderItem={({ item }: { item: PendingMessageType }) => (
          <View style={s.card}>
            ...
            <View style={s.encNote}>
              <LockSimpleIcon size={15} color="#20C7B3" weight="bold" />
...
const s = StyleSheet.create({
  ...
  brandTxt:   { color: '#0FA79A', fontSize: 10, fontWeight: '800', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 2 },
  title:      { color: '#F1F5F9', fontSize: 26, fontWeight: '800', letterSpacing: -0.5 },
  desc:       { color: '#6D8298', fontSize: 13, lineHeight: 18, paddingHorizontal: 20, paddingBottom: 16 },
  ...
  time:       { color: '#6D8298', fontSize: 12, marginTop: 2 },
  encNote:    { flexDirection: 'row', alignItems: 'center', backgroundColor: '#0D1626', borderRadius: 10, padding: 10, marginBottom: 14, gap: 8 },
  encNoteTxt: { color: '#0FA79A', fontSize: 12, flex: 1, lineHeight: 16 },
  btnRow:     { flexDirection: 'row', gap: 8 },
  dismissBtn: { flex: 1, backgroundColor: '#102131', borderRadius: 12, paddingVertical: 12, alignItems: 'center', borderWidth: 1, borderColor: '#1B2F43' },
  dismissTxt: { color: '#64748B', fontSize: 14, fontWeight: '600' },
  acceptBtn:  { flex: 2, backgroundColor: '#0FA79A', borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
  ...
  emptyDesc:  { color: '#6D8298', fontSize: 14, textAlign: 'center', lineHeight: 20 },
})
