...
export default function RoomScreen() {
  ...
  useEffect(() => {
    const loadMuted = async () => {
      if (roomType !== 'dm' || !otherMember?.id) {
        setIsDmMuted(false)
        return
      }
      setIsDmMuted(await storage.isUserMuted(otherMember.id))
    }
    loadMuted().catch(() => {})
  }, [roomType, otherMember?.id])
  ...
  const handleToggleDmMute = async () => {
    if (!otherMember) return
    try {
      const next = !isDmMuted
      await storage.setUserMuted(otherMember.id, next)
      setIsDmMuted(next)
      Alert.alert('Thông báo', next ? `Đã tắt thông báo @${otherMember.username}` : `Đã bật lại thông báo @${otherMember.username}`)
    } catch (e) {
      Alert.alert('Lỗi', e instanceof Error ? e.message : 'Không đổi được trạng thái thông báo')
    }
  }
      ...
      <View style={[s.header, { paddingTop: Math.max(insets.top, 8) + 6 }]}>
        ...
        {roomType === 'dm' && otherMember && callState === 'idle' && groupCallState === 'idle' && (
          <TouchableOpacity
            style={s.headerBtn}
          onPress={() => {
            startCall(otherMember.id, otherMember.username, roomId).catch((err) => {
              Alert.alert('Lỗi cuộc gọi', err instanceof Error ? err.message : 'Không thể bắt đầu cuộc gọi')
            })
          }}
            activeOpacity={0.7}
          >
      ...
      {roomType === 'group' && (
        <GroupCallScreen
          ...
          onJoin={() => {
            ...
            joinGroupCall(myUsername).catch((err) => {
              Alert.alert('Lỗi cuộc gọi nhóm', err instanceof Error ? err.message : 'Không thể tham gia cuộc gọi')
            })
          }}
          ...
          <View style={mm.menu}>
            {roomType === 'dm' && otherMember && (
              <>
                ...
                <TouchableOpacity style={mm.item} onPress={() => { setMenuModal(false); handleToggleDmMute() }}>
                  <BellSlashIcon size={17} color="#E2E8F0" weight="bold" />
                  <Text style={mm.itemTxt}>{isDmMuted ? `Bật thông báo @${otherMember.username}` : `Tắt thông báo @${otherMember.username}`}</Text>
                </TouchableOpacity>
              </>
            )}
            {roomType === 'group' && (
