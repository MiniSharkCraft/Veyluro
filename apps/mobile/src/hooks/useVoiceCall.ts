  ...
  const handleSignal = useCallback(async (msg: any) => {
    try {
      if (msg.type === 'call-ring') {
        if (!msg?.fromUserId || !msg?.roomId || !msg?.data?.offer) return
        notifyIncomingCall(`@${msg.data?.fromUsername ?? msg.fromUserId}`).catch(() => {})
        setIncomingCall({
      ...
      if (msg.type === 'call-answer' && pcRef.current && msg?.data) {
        await pcRef.current.setRemoteDescription(new RTCSessionDescription(msg.data))
        setCallState('active')
      }

      if (msg.type === 'call-ice' && pcRef.current && msg?.data) {
      ...
      console.warn('[voice-call] signal error:', err)
    }
  }, [cleanup])
