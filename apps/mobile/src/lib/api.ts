...
export const messagesApi = {
  ...
  uploadImage: async (
    ...
    options?: { encrypted?: boolean; originalMime?: string; originalName?: string }
  ) => {
    const token = await storage.getToken()
    if (!token) throw new ApiError(401, 'Chưa đăng nhập')
    const body = new FormData()
    ...
    const res = await fetch(`${BASE}/api/messages/${roomId}/attachments`, {
      ...
      headers: {
        Accept: 'application/json',
        Authorization: `Bearer ${token}`,
        ...(options?.encrypted ? { 'X-AMOON-E2EE': '1' } : {}),
        ...(options?.originalMime ? { 'X-AMOON-ORIGINAL-MIME': options.originalMime } : {}),
        ...(options?.originalName ? { 'X-AMOON-ORIGINAL-NAME': options.originalName } : {}),
      },
      body,
    })
    if (!res.ok) {
